<?php
/**
 * Magento 2 extensions for Splitoff
 *
 * @author Splitoff
...
use Splitoff\TogetherPay\Model\Adapter\V2\SplitoffOrderTokenV2 as SplitoffOrderTokenV2;
...
class Process extends \Magento\Framework\App\Action\Action
{
    ...
    protected $_splitoffOrderTokenV2;
    ...
    public function __construct(
        ...
        SplitoffOrderTokenV2 $splitoffOrderTokenV2,
        ...
        $this->_splitoffOrderTokenV2 = $splitoffOrderTokenV2;
    ...
    public function _processAuthorizeCapture()
    {
        ...
        $website_id = $this->_splitoffConfig->getStoreObjectFromRequest()->getWebsiteId();
        ...
        $payment->setMethod(\Splitoff\TogetherPay\Model\Payovertime::METHOD_CODE);

        $quote->reserveOrderId();

        try {
            $payment = $this->_getSplitoffOrderToken($this->_splitoffOrderTokenV2, $payment, $quote);
        } catch (\Exception $e) {
            $result = $this->_jsonResultFactory->create()->setData(
                ['error' => 1, 'message' => $e->getMessage()]
            );
        ...
        $merchantId = $this->scopeConfig->getValue('payment/splitoffpayovertime/merchant_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);

        $result = $this->_jsonResultFactory->create()->setData(
            ['success' => true, 'token' => $token, 'merchantId' => $merchantId]
        );
     ...
     */
    private function _getSplitoffOrderToken($splitoffOrderToken, $payment, $targetObject)
    {
        if ($targetObject && $targetObject->getReservedOrderId()) {
            $result = $splitoffOrderToken->generate($targetObject, ['merchantOrderId' => $targetObject->getReservedOrderId() ]);
        } elseif ($targetObject) {
            $result = $splitoffOrderToken->generate($targetObject);
        }
        ...
        if ($orderToken) {
            $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN, $orderToken);
        } else {
