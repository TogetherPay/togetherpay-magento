...
class Response
{
     ...
     * @param Adapter\SplitoffPayment $splitoffApiPayment
    ...
    public function __construct(
        ...
        $this->splitoffApiPayment = $splitoffApiPayment;
        $this->helper = $helper;
    ...
    public function validCallback(\Magento\Sales\Model\Order $order, $response = [])
    {
        ...
        if ($this->request->getParam('orderToken') ===  $additionalInfo[\Splitoff\TogetherPay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN]) {
            return true;
        }
        return false;
    }
        ...
        if ($order->canInvoice() && $this->_shouldInvoiced($order, $orderId)) {
            ...
            switch ($this->status) {
                case \Splitoff\TogetherPay\Model\Status::STATUS_APPROVED:
                    ...
                    break;
                case \Splitoff\TogetherPay\Model\Status::STATUS_PENDING:
                    $order->addStatusHistoryComment(__('Payment under review by Splitoff'));
    ...
    {
        // adding Splitoff order id to the payment
        $payment = $order->getPayment();
        $payment->setTransactionId($orderId);
        $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_ORDERID, $orderId);
        // have save here to link splitoff order id right after checking the API
        $this->_paymentRepository->save($payment);

        // debug mode
        $this->helper->debug('Added TogetherPay Payment ID ' . $orderId . ' for Magento order ' . $order->getIncrementId());
    }
    ...
    public function createInvoiceAndUpdateOrder(\Magento\Sales\Model\Order $order, $orderId)
    {
        ...
        $order->addStatusHistoryComment(__('Payment approved by TogetherPay'));
     ...
     * @return bool
     */
    protected function _shouldInvoiced(\Magento\Sales\Model\Order $order, $splitoffOrderId)
    {
        ...
        // only process splitoff method
        if ($order->getPayment()->getMethod() !== \Splitoff\TogetherPay\Model\Payovertime::METHOD_CODE) {
            return false;
        }

        // checking with API to make sure the payment exist with correct status in API
        $response = $this->splitoffApiPayment->getPayment($splitoffOrderId);
        $response = $this->jsonHelper->jsonDecode($response->getBody());

        $this->status = $response['status'];

        if ($response['token'] == $order->getPayment()->getAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN) &&
            ($response['status'] == \Splitoff\TogetherPay\Model\Status::STATUS_APPROVED || $response['status'] == \Splitoff\TogetherPay\Model\Status::STATUS_PENDING)
        ) {
            return true;
        }
    ...
    public function     calculateRefund($payment, $amount)
    {
        ...
        if($orderId) {
            ...
            if($splitoffPaymentStatus == self::PAYMENT_STATUS_CAPTURED){
                $splitoffRefund = true;
            }
            elseif($splitoffPaymentStatus == self::PAYMENT_STATUS_PARTIALLY_CAPTURED || $splitoffPaymentStatus == self::PAYMENT_STATUS_AUTH_APPROVED){
                ...
                $rolloverDiscount          = $payment->getAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ROLLOVER_DISCOUNT);
                ...
                else
                {
                    ...
                    if($appliedDiscount > 0){
                        ...
                        if($rolloverDiscount > 0){
                            $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ROLLOVER_DISCOUNT, max(($rolloverDiscount - $reducedRolloverDiscount),"0.00"));
                        }
                        else{
                    ...
                    elseif($amountToRefund < $actualOpenToCaptureAmount && $amountToRefund != 0.00){
                        if($order->getShipmentsCollection()->count()==0){
                            ...
                            elseif(number_format($amountInclShipping -  $orderTotal, 2, '.', '') == 0.00 || number_format($amountInclShipping -  $actualOpenToCaptureAmount, 2, '.', '') == 0.00){
                                ...
                                $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::ROLLOVER_REFUND, number_format($rolloverRefund, 2, '.', ''));
                                $result['success'] = true;
                            }
                        }
                        else{
            ...
            $result['success'] = $this->splitoffProcessRefund($payment,$order,$amountToCapture,$splitoffRefund,$amount,$splitoffVoid,$orderId,$override);
            ...
            throw new \Magento\Framework\Exception\LocalizedException(__('There are no Splitoff payment linked to this order. Please use refund offline for this order.'));
        }
        return $result;
    }
    /**
     ...
     * @return array
     */
    public function splitoffProcessRefund($payment,$order,$amountToCapture,$splitoffRefund,$amount,$splitoffVoid,$orderId,$override)
    {
        $success = false;
        //Capture request
        if($amountToCapture > 0){
            ...
            if(!array_key_exists("errorCode",$captureResponse)) {
                $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::PAYMENT_STATUS,$captureResponse['paymentState']);
                if(array_key_exists('openToCaptureAmount',$captureResponse) && !empty($captureResponse['openToCaptureAmount'])){
                    $payment->setAdditionalInformation(\Splitoff\TogetherPay\Model\Payovertime::OPEN_TOCAPTURE_AMOUNT,number_format($captureResponse['openToCaptureAmount']['amount'], 2, '.', ''));
                }
                $success = true;
            }
            else{
                $this->helper->debug("Transaction Exception : " . json_encode($captureResponse));
                throw new \Magento\Framework\Exception\LocalizedException(__('Splitoff API Error: ' .$captureResponse['message']));
            }
        }
        //Refund reqest
        if($splitoffRefund && $amount > 0){

            $refundResponse = $this->splitoffApiPayment->refund(number_format($amount, 2, '.', ''),$orderId,$order->getOrderCurrencyCode(),$override);
            ...
            if (!empty($refundResponse['refundId'])) {
                ...
                $this->helper->debug('TogetherPay API Error: ' . $refundResponse['message']);
                throw new \Magento\Framework\Exception\LocalizedException(__('TogetherPay API Error: ' .$refundResponse['message']));
            }
        }

        if($splitoffVoid){
            //Void request
            $voidResponse = $this->splitoffApiPayment->voidOrder($orderId,$override);
            ...
            if(!array_key_exists("errorCode",$voidResponse)) {
                ...
                throw new \Magento\Framework\Exception\LocalizedException(__('TogetherPay API Error: ' .$voidResponse['message']));
            }
        }
        return $success;
    }
