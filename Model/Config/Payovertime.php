 ...
 */
namespace Splitoff\TogetherPay\Model\Config;

use Splitoff\TogetherPay\Model\Adapter\ApiMode;
...
/**
 * Class Payovertime
 * @package Splitoff\TogetherPay\Model\Config
 */
class Payovertime
{
    /**
    ...
    */
    ...
    public function getSiteConfig($apiMode, $type, $websiteId)
    {
        ...
        if ($type=='api_url') {
            if ($apiMode == 'Sandbox') {
                if ($currency == 'USD' || $currency == 'CAD') {
                    //$url = 'https://api.us-sandbox.splitoff.io/';
                    $url = 'http://develop.splitoff.io/';
                } else {
                    //$url = 'https://api-sandbox.splitoff.io/';
                    $url = 'http://develop.splitoff.io/';
                }
            } elseif ($apiMode == 'Production') {
                if ($currency == 'USD'  || $currency == 'CAD') {
                    //$url = 'https://api.us.splitoff.io/';
                    $url = 'http://develop.splitoff.io/';
                } else {
                    //$url = 'https://api.splitoff.io/';
                    $url = 'http://develop.splitoff.io/';
                }
            }
        }
        ...
        if ($type=='web_url') {
            if ($apiMode == 'Sandbox') {
                //$url = 'https://portal.sandbox.splitoff.io/';
                $url = 'http://splitoff.io/pay/develop/';
            } elseif ($apiMode == 'Production') {
                //$url = 'https://portal.splitoff.io/';
                $url = 'http://splitoff.io/pay/develop/';
            }
        }
        ...
        if ($type=='js_lib_url') {
            if ($apiMode == 'Sandbox') {
                $url = 'https://js.sandbox.splitoff.io/';
            } elseif ($apiMode == 'Production') {
                $url = 'https://js.splitoff.io/';
            }
        }
        return $url;
    }
    ...
    protected function _getConfigData($path, $override = [])
    {
        ...
        if (!empty($website_id) && $website_id) {
            return $this->scopeConfig->getValue('payment/' . \Splitoff\TogetherPay\Model\Payovertime::METHOD_CODE . '/' . $path, \Magento\Store\Model\ScopeInterface::SCOPE_WEBSITES, $website_id);
        } else {
            // var_dump($this->scopeConfig->getValue('payment/' . \Splitoff\TogetherPay\Model\Payovertime::METHOD_CODE . '/' . $path, 'default'));
            return $this->scopeConfig->getValue('payment/' . \Splitoff\TogetherPay\Model\Payovertime::METHOD_CODE . '/' . $path, 'default');
        }
    }
